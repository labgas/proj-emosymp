...
% LaBGAS_extract_confound_reg_fMRIprep.m - DEFAULT
% 'CANlab'
...
% in the workflow in the future (by deleting the option there rather than
% here, as that would allow you to entirely skip that script if you want to
...
%
%__________________________________________________________________________
% @(#)% LaBGAS_1_spm_prep_firstlvl_models.m         v1.3        
...
for i = 1:size(DSGN.subjects,2) % iter over subj
    ...
    nii = dir([DSGN.subjects{i},'\',DSGN.funcnames{j}]);
        if ~isempty(nii) % if has nii
            ...
            % if relevant, define and load pmods file generated by
            ...
            if isfield(DSGN,'pmods')
                negative.pmod = struct('name',{pmods2model(1)},...
                    'param',{{[]}},...
                    'poly',{{[1]}});
                neutral.pmod = struct('name',{pmods2model(2)},...
                    'param',{{[]}},...
                    'poly',{{[1]}});
                positive.pmod = struct('name',{pmods2model(3)},...
                    'param',{{[]}},...
                    'poly',{{[1]}});
                for z = 1:size(pmod_dat.trial_valence,1)
                    switch pmod_dat.trial_valence(z)
                        case 'neg'
                            negative.pmod.param{1} = [negative.pmod.param{1}, pmod_dat.state_symptoms(z)];
                        case 'neu'
                            neutral.pmod.param{1} = [neutral.pmod.param{1}, pmod_dat.state_symptoms(z)];
                        case 'pos'
                            positive.pmod.param{1} = [positive.pmod.param{1}, pmod_dat.state_symptoms(z)];
                    end
                end
