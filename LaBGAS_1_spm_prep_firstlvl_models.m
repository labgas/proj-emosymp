...
for i = 1:size(DSGN.subjects,2) % iter over subj
    ...
    onsetfiles=ls([nii(1).folder,'\onset*']);
    for j = 1:size(nii,1) % iter over scans/niftis
        if ~isempty(nii) % if has nii
            save_files = true;
            
            nii_path = [nii(1).folder, '\', nii(j).name];
            nii_unzip = char(gunzip(nii_path)); % unzip; nii.gz not supported by CANlab read_hdr and scn_session_spike_id functions below
            nii_path = [nii(1).folder, '\', nii_unzip];
            sid = nii(j).name(4:16);
            ...
            try 
                stim_dat = importdata([nii(1).folder,'\',onsetfiles(j,:)]); % in our case, this generates a table variable
            catch
            ...
            if strcmpi(noise_method,'CANlab')==1
                ...
                try
                    fid = fopen([noise_tsv(1).folder,'\',noise_tsv(j).name]);
            ...
            if save_files
                mkdir(nii(j).folder,DSGN.modelingfilesdir)
                mdl_path = [nii(j).folder, '\', DSGN.modelingfilesdir, '\'];
            ...
            warning(strcat('missing run_',num2str(j),' in subject_',subject)); % runs are numbered consecutively, but this error should never apply
        end
    end
end
