...
for =1:size(subjs,1)
    ...
    %% define subjectdirs
    subjderivdir=fullfile(derivdir,subjs{h},'func');
    subjrawdir=fullfile(rawdir,subjs{h},'func');
    ...
    runnames = runnames(:,1:24); % this is study-specific, depends on length of taskname - could be adapted based on regexp, but easy enough to adapt per study
    ...
    for task=1:length(noisefiles)
        % load confound regressor file generated by fMRIprep into Matlab table
        % variable
        cd(derivdir);
        R=readtable(noisefiles{task},'TreatAsEmpty','n/a','FileType', 'text', 'Delimiter', 'tab');
        ...
        filename = strcat('noise_regs_',runnames(task,1:15));
        ...
        filename = strcat('onsets_',runnames(task,1:15));
        cd(outputdir);
        save(filename,'O');
    end
